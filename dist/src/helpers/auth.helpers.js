...
const checkOtpRestrictions = (email, next) => __awaiter(void 0, void 0, void 0, function* () {
    if (yield index_1.default.get(`otp_lock:${email}`)) {
        return next(new error_helpers_1.ValidationError("Account locked due multiple failed attempts! Try again after 30 minutes."));
    }
    if (yield index_1.default.get(`otp_spam_lock:${email}`)) {
        return next(new error_helpers_1.ValidationError("Too many OTP requests! Please wait 1 hour before requesting again."));
    }
    if (yield index_1.default.get(`otp_cooldown:${email}`)) {
        return next(new error_helpers_1.ValidationError("Please wait 1 minute before requesting a new OTP."));
    }
});
...
const trackOtpRequest = (email, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (otpRequests >= 2) {
        ...
        throw new error_helpers_1.ValidationError("Too many OTP requests. Please wait 1 hour before requesting again");
    }
...
const verifyOtp = (email, otp, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!storedOtp)
        throw new error_helpers_1.ValidationError("Invalid or expired OTP!");
    ...
    if (storedOtp !== otp) {
        if (failedAttempts >= 2) {
            ...
            throw new error_helpers_1.ValidationError("Too many failed attempts. You account is locked for 30 minutes.");
        }
        yield index_1.default.set(failedAttemptsKey, failedAttempts + 1, "EX", 300);
        throw new error_helpers_1.ValidationError(`Inccorect OTP. ${2 - failedAttempts} attemps left.`);
    }
    yield index_1.default.del(`otp:${email}`, failedAttemptsKey);
});
